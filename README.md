...
To update the database use this command from the Web project folder (replace `E_MoveBCN_back` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../E_MoveBCN_back.Infrastructure/E_MoveBCN_back.Infrastructure.csproj -s E_MoveBCN_back.Web.csproj
```
