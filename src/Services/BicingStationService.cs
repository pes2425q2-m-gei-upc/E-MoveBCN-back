...
public class BicingStationService : IBicingStationService
{
  ...
  {
    return await _bicingStationRepository.GetAllBicingStations().ConfigureAwait(false);
  }
  ...
  public async Task FetchAndStoreBicingStationsAsync()
  {
    try
    {
      ...
      var bicingStationsToAdd = await ProcessStationsDataAsync().ConfigureAwait(false);
      await _bicingStationRepository.BulkInsertAsync(bicingStationsToAdd).ConfigureAwait(false);
