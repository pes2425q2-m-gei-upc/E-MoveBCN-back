...
public class ChargingStationsService : IChargingStationsService
{
  ...
  {
    return await _chargingStationsRepository.GetAllChargingStations().ConfigureAwait(false);
  }
  ...
  public async Task FetchAndStoreChargingStationsAsync()
  {
    try
    {
      ...
      await _chargingStationsRepository.BulkInsertAsync(locationsToAdd, hostsToAdd, stationsToAdd, portsToAdd).ConfigureAwait(false);
